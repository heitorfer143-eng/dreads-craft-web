...
func run() -> void:
	...
	check(scene.login_root.find_child("LoginCard",true,false)==null,"Legacy second login card is removed")
	...
	check(scene.has_method("setup_web_login_overlay") and scene.has_method("poll_web_login") and scene.has_method("continue_as_guest"),"Single browser login and guest flow exist")
